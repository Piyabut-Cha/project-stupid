            ...
            if (response.RespCode == 200) {
                ...
                for (let i = 0; i < product.length; i++) {
                    html += `<div onclick="openProductDetail(${i})" class="product-items ${product[i].type}">
                                ...
                                <p style="">${product[i].name}</p>
                                <p stlye="">${numberWithCommas(product[i].price)} THB</p>
                            </div>`;
                }
...
function searchsomething(elem) {
    ...
    if (html == '') {
        $("#productlist").html(`<p>ไม่พบสินค้าดังกล่าว</p>`);
    } else {
        $("#productlist").html(html);
    }
...
function addtocart() {
    ...
    Swal.fire({
        icon: 'success',
        title: 'Add ' + product[productindex].name + ' to calculator !'
    })
